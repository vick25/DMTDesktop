...
import com.osfac.dmt.workbench.ui.task.TaskMonitorDialog;
...
/**
 * Overrides most of the abstract methods of the AbstractPlugIn to implement a
 * default behavior, that fits the needs for a plugIn in the PIROL context. Also
 * implements the ErrorHandler interface and offers methods to post error
 * messages to the GUI.
 *
 * @author Ole Rahn, Stefan Ostermann <br> <br>FH Osnabr&uuml;ck - University of
 * Applied Sciences Osnabr&uuml;ck, <br>Project: PIROL (2005), <br>Subproject:
...
public abstract class StandardPirolPlugIn extends AbstractPlugIn implements ErrorHandler {
    ...
    /**
     * buffer for messages of errors that occured during execution - needed to
     * implement the ErrorHandler interface
     */
    protected StringBuffer messages = new StringBuffer();
    /**
     * Are errors to be shown in the statusbar (bringUpMessages = false,
     * DEFAULT) or in an output window (bringUpMessages = true)?
     */
    ...
    /**
     * logger reference for use with the StandardPirolPlugIn. Since loggers are
     * personalized, this member has to be initialized in derived classes by the
     * programmer, before it can be used. For downward compatibility no abstract
    ...
    /**
     * Method to enable loading an icon from the surrounding jar-file if
     * necessary. If the plugin doesn't need an icon, just return
     * <code>null</code> else return the file name, e.g. "xyz.png" and put the
     * picture in the same folder (package) as the deriving class.
     *
     * @return the filename of the icon or <code>null</code> if the plugin has
     * no icon.
     */
     ...
     * @throws Exception
     * @see
     * com.osfac.dmt.workbench.plugin.AbstractPlugIn#execute(com.osfac.dmt.workbench.plugin.PlugInContext)
     */
     ...
     * Looks for a key (the PlugIn's name with no path) in the i18n resources,
     * if none is found the standard jump name generation will be used.
     */
     ...
     * <code>PIROL Tools/<b>[return value of getCategoryName()]</b></code> menu
     * and a standard {@link com.osfac.dmt.workbench.plugin.EnableCheck} is
     * applied.
     *
    ...
    /**
     * The name of the category, a PlugIn can be found in - this method should
    ...
    /**
     * Standard intialization: PlugIn is added to the
     * <code>PIROL Tools/<b>subMenuName</b></code> (or localized menu name) menu
     * and a standard {@link com.osfac.dmt.workbench.plugin.EnableCheck} is
     * applied.
     *
     * @deprecated
    ...
    /**
     * Writes the given message and function string to the stdout. The output
     * will be formated as "classname.function: message".
     *
     ...
     * @param num max. number of layers to return, -1 returns all selected
     * layers
    ...
    /**
     * To be called, when leaving the execute()-method. Puts out the error
     * messages collected during execution if any and returns the given boolean
     * value.
	 * Example:
     * <pre>return finishExecution(context, true);</pre>
     *
     * @param context the current PlugInContext
     ...
     *
     * @return the icon or <code>null</code> if the getIconString() returned
     * null (or problems occured)
     */
    public Icon getIcon() {
    ...
    /**
     * Get a List of Features (from the given Layer) that reside within the
     * fence (if there is a fence) or just the features of the layer (if there
     * is currently no fence)
     *
