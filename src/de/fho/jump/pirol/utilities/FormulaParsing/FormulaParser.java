 ...
 *  $Author: ma15569 $
 *  $Rev: 2551 $
 *  $Date: 2011-12-17 11:39:24 +0100 (sam., 17 déc. 2011) $
 *  $Id: FormulaParser.java 2551 2011-12-17 10:39:24Z ma15569 $
 */
...
/**
 * This class is a utility to parse formulas, that describe how an additional
 * attribute value is to be calculated on a by feature basis. Formulas thereby
 * can contain constant values as well as attribute values, that need to be
 * extracted for each feature.<br><br>
 *
 * Formulas are exspected to be space-separated: Each attribute name, constant
 * value, bracket or operator has to be surrounded by empty spaces.<br><br> A
 * valid formulas (for a FeatureSchema that has the attributes "yield" and
 * "grain mois") would look like this: <br><code>( 4 + 6 ) * yield</code><br>
 * or<br><code>grain mois / 2</code>.
 *
 ...
 *
 * @see
 * de.fho.jump.pirol.utilities.FeatureCollectionTools#applyFormulaToFeatureCollection(FeatureCollection,
 * AttributeInfo, FormulaValue, boolean)
 *
 */
public class FormulaParser {
    ...
    /**
     * Recursively parses a given (sub-) formula into a FormulaValue, which can
     * be an operation with sub-FormularValues or a value.
     *
     * @param formula
     * @param featSchema The feature schema to check attribute names, if
     * neccessary
     * @return the given formula parsed into a FormulaValue or null if the given
     * String did not contain formula information
     */
    ...
    protected static int findFirstOccuranceOutsideABracket(String toBeFound, String formula, int fromIndex) {
        ...
        for (int i = Math.max(0, fromIndex); i < characters.length; i++) {
            ...
            if (characters[i] == '(') {
                ...
                numQuote++;
            } else if (characters[i] == char2bFound && bracketOpen == bracketClose && numQuote % 2 == 0) {
                return i;
            }
        }
        return -1;
