...
import com.osfac.dmt.io.EndianDataInputStream;
import java.io.*;
import java.net.URL;
...
/**
 *
 * This class represents a DBF (or DBase) file.<p> Construct it with a URL or
 * File (including the .dbf) this causes the header and field definitions to be
 * read.<p> Later queries return rows or columns of the database.<p> If a URL is
 * specified then the whole file is read into memory<br> if a file is specified
 * then a randomAccess system is used.<br> <hr>
 *
 * @author <a href="mailto:ian@geog.leeds.ac.uk">Ian Turton</a> Centre for
 * Computaional Geography, University of Leeds, LS2 9JT, 1998. <br> mod to
 * getStringCol by James Macgill.
 */
public class Dbf implements DbfConsts {
    ...
    public Vector ParseRecord(StringBuffer rec) {
        ...
        for (int i = 0; i < numfields; i++) {
            ...
            if (DEBUG) {
                System.out.println(DBC + "" + t.substring(fielddef[i].fieldstart,
            ...
            switch (fielddef[i].fieldtype) {
                ...
                default:
                    if (DEBUG) {
                        System.out.println(DBC + "Oh - don't know how to parse "
    ...
    public Integer[] getIntegerCol(int col, int start, int end)
        ...
        try {
            for (i = start; i < end; i++) {
                ...
                record = sb.toString();
                if (DEBUG) {
                    System.out.println(DBC + record.substring(fielddef[col].fieldstart,
