...
public class SaveImageAsSVGPlugIn extends AbstractPlugIn implements ThreadedPlugIn {
    ...
    public void run(TaskMonitor monitor, PlugInContext context)
        ...
        /**
         * old working code for original jump // paint using new renderer
         * LayerViewPanel lvp = context.getLayerViewPanel(); RenderingManager rm
         * = lvp.getRenderingManager(); Class[] types1 = {Object.class,
         * Renderer.class}; List layers =
         * context.getLayerManager().getVisibleLayers(false); for (int i=0; i <
         * layers.size(); i++) { Layer layer = (Layer)layers.get(i); //-- do now
         * this: // rm.setRenderer(layer,new SvgRenderer(layer,
         * context.getLayerViewPanel())); // but for an invisible method
         * setRenderer() SvgRenderer sr = new SvgRenderer(layer,
         * context.getLayerViewPanel()); Object[] params1 ={layer, sr};
         * AccessToPrivateMethods.invokePrivateMethod("setRenderer",rm,RenderingManager.class,params1,types1);
         * sr.createRunnable(); //do paint (also on screen) } //paint the
         * layerview into the svgGenerator lvp.paintComponent(svgGenerator);
		*
         */
