...
public class SaveDatasetsPlugIn extends AbstractPlugIn {
    ...
    public boolean execute(PlugInContext context) throws Exception {
        try {
            ...
            FileFilter fileFilter3 = GUIUtil.createFileFilter("DMT " + sFiles, new String[]{"dmt"});
            ...
            for (int i = layerList.size() - 1; i >= 0; i--) {
                ...
                if (layer.getFeatureCollectionWrapper().getFeatures().isEmpty()) //layer is empty
                {
    ...
    private void writeProjectionFile(PlugInContext context, Layer outputLayer) throws IOException, FileNotFoundException {	//per LDB projection files only associated with .shp files; confirmed 8/16/05
        ...
        if (dsqOut != null
            ...
            if ((outputFileName.toLowerCase()).endsWith(".shp")) {
                ...
                if (!(new File(outputPrjFileName).exists())) {	//loop through all layers to find a project file; then copy contents
