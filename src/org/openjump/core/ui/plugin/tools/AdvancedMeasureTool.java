...
import com.osfac.dmt.workbench.ui.cursortool.PolygonTool;
...
public class AdvancedMeasureTool extends PolygonTool implements ActionListener {
    ...
    private JideToggleButton toolbarButton = null;
    ...
    public AdvancedMeasureTool(WorkbenchContext context) {
        ...
        optionsMenuItem.setIcon(DMTIconsFactory.getImageIcon(DMTIconsFactory.Standard.SETTING));
    ...
    /**
     * Clickhandler for the measuretool. If the user starts a new measure with a
     * sinle click, then all old mesurements (features on the mesure layer) will
     * be deleted. But if the user starts with a double click, then this
     * measuremt will be added! Because of the event behaviour during a double
     * click, this code is a little bit tricky. During a double click you get
     * first a single click (getClickCount()=1) and after them the double click
     * (getClickCount()=2). So we must check after the first click if later
     * comes the double click. This is done by a Timer. The maximum time to
     * detect a double click we can get through the desktop property
     * "awt.multiClickInterval". Second is the overridden method
     * MultiClickTool.isFinishingRelease() important.
     *
     * @param e
     */
    @Override
     ...
     * only finish, if we have more then one click previously done (coordinates
     * > 1). See mouseClicked() method.
     *
     * @param e
     ...
     */
    public JideToggleButton getToolbarButton() {
        return toolbarButton;
    }

    /**
     * ActionListener for the JMenuItems. We must set the Icon, the TooltipText,
     * Cursor and the CloseRing Mode. Second the tool will be activated through
     * a JMenuItem ActionEvent. So you do not need a second click ;-)
     *
     * @param e
     */
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == distanceMenuItem) { // Distance
            ...
            GeneralPanel.TabbedPaneAll.setSelectedIndex(0);
