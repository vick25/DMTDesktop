 ...
 * @version 4 May 2008
 *
 * Merges/Intersects two polygon layers into one layer. It therefore calculates all geometric
 * intersections between the polygons. Afterwards the attributes are transferred. The later step
 * assumes that a new created intersection polygon has at max only one correspondent polygon per
 * layer. TODO : abstract methods to be able to intersect not only two polygon layers TODO:
...
public class IntersectPolygonLayersPlugIn extends ThreadedBasePlugIn {
    ...
    /**
     * Merges/Intersects two polygon layers into one layer. It therefore extracts all lines of the
     * polygons and creates new polygons with the Polygonizer class. Afterwards it is checked which
     * polygons have a correspondent in the input layers. If the polygon does not have it is removed
     ...
     * @return a FeatureCollection that contains the result of the intersection (i.e. the new
     * created features)
     */
...
//			}
     ...
     * intersections between the polygons. Afterwards the attributes are transferred. The later step
     * assumes that a new created intersection polygon has at max only one correspondent polygon per
     * layer. Note: this approach using the IntersectGeometries() class results in unwanted spikes
     * during the polygon intersection. The method runIntersectionNew() tries to avoid this by not
     * using the polygon intersection but creating polygons from all lines derived from the polygons
     * and removing some of those.
     *
     * @param fcA
     * @param fcB
     * @param accuraccy this parameter has currently no effect (now the default fixed precision
     * model is used)
