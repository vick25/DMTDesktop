 ...
 * @author sstein
 *
 * url www.openjump.org curentdate 27 May 2010 license GPL2
 *
 * Intersects polygons in one layer. It therefore calculates all geometric intersections between the
 * polygons. Afterwards the attributes are transferred. The later step assumes that a new created
...
public class IntersectPolygonsOneLayerPlugIn extends ThreadedBasePlugIn {
    ...
    /**
     * Merges/Intersects polygons in one layer. It therefore extracts all lines of the polygons and
     * creates new polygons with the Polygonizer class. Afterwards it is checked which polygons have
     * a correspondent in the input layers. If the polygon does not have it is removed otherwise the
     * attributes are transferred. The later step assumes that a new created intersection polygon
     * has at max only one correspondent polygon per layer.
     *
     * @param fcA
