...
public class BeanToolsPlugIn extends AbstractPlugIn {
    ...
    private void scanBeanShellDir(final File file, final PlugInContext context) throws IOException {
        ...
        if (file.isDirectory()) {
            ...
            JideMenu menu = featureInstaller.menuBarMenu(MenuNames.CUSTOMIZE);
            if (menu == null) {
                menu = (JideMenu) featureInstaller.installMnemonic(new JideMenu(I18N.get(MenuNames.CUSTOMIZE)), featureInstaller.menuBar());
                featureInstaller.menuBar().add(menu);
            }
            JideMenu parent = featureInstaller.createMenusIfNecessary(menu, ancestors.split(File.separator.replace("\\", "\\\\")));
