...
public class RasterImageLayer extends AbstractLayerable implements ObjectContainingMetaInformation {
     ...
     * (envelope, etc.) changes.<br> default: true
     */
    protected boolean firingAppearanceEvents = true;
    /**
     * Flag to control if the image should be deleted from RAM as soon as
     * possible to save RAM or if it should be keeped e.g. because it was
     * generated dynamically and can not be loaded from a file again, once it
     * was deleted.
     */
     ...
     * there is no file name, but an image
     *
     * @param name name of the layer
     * @param layerManager
     * @param imageToDisplay the image (if already loaded) or null
     ...
     * consumption is high) and calls the garbage collector, if the
     ...
     * <code>Point</code> object. The clue is that only the image file's header
     * is read to get this information, so it's quite fast, because the image
    ...
    /**
     * Method to change the coordinates of the image and later apply the changes
     * to the RasterImageLayer by using
     * {@link RasterImageLayer#setGeometryAsEnvelope(Geometry)}.
     *
    ...
    /**
     * Add transparency to the image (more exactly: to each pixel which a color
    ...
    private static boolean tilesAreNotNullAndCongruent(Rectangle r1, Rectangle r2) {
        ...
        if (r1 == null || r2 == null) {
            ...
            result = (r1.x == r2.x && r1.y == r2.y && r1.width == r2.width && r1.height == r2.height);
        }
        return result;
    }
     ...
     * controlls the transparency level of the whole image (all pixels). It is
     * independent of the transparency color, that replaces a certain color in
     * the image. The transparencyLevel is expressed as a float within a range
    ...
    /**
     * Sets the transparency level of the image. This controlls the transparency
     * level of the whole image (all pixels). It is independent of the
     * transparency color, that replaces a certain color in the image. The
     ...
     * level of the whole image (all pixels). It is independent of the
     * transparency color, that replaces a certain color in the image. The
     * transparencyLevel is expressed as a percentage within a range from 0 (no
    ...
    /**
     * After this method was invoked, the image will be completely re-rendered
     * (not using caches) the next time.
     */
