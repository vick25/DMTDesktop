...
/**
 * User: geoff Date: 17/06/2007 Time: 09:39:05 Copyright 2005 Geoffrey G Roy.
 */
public class LoadImages extends Component implements FilenameFilter, Runnable {
    ...
    private void loadNames() {
        File file = new File(new StringBuilder(wd).append(File.separator).append("VertexImages").toString());
        if (!file.exists()) {
            file.mkdirs();
        }
        if (debug) {
        ...
        for (int i = 0; i < fileNames.length; i++) {
            if (!fileNames[i].toLowerCase().endsWith(".wkt")) {
                ...
                images[j] = loadImage(new StringBuilder(wd).append(File.separator).append("VertexImages")
                        .append(File.separator).append(fileNames[i]).toString());
                j++;
            }
        }
        j = 0;
        for (int i = 0; i < fileNames.length; i++) {
            if (fileNames[i].toLowerCase().endsWith(".wkt")) {
                ...
                wktShapes[j] = loadWKT(new StringBuilder(wd).append(File.separator).append("VertexImages")
                        .append(File.separator).append(fileNames[i]).toString());
                j++;
            }
        }
