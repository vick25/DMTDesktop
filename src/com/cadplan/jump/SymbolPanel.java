...
/**
 * User: geoff Date: 16/06/2007 Time: 13:26:45 Copyright 2005 Geoffrey G Roy.
 */
public class SymbolPanel extends JPanel {
    ...
    public int getIndex(int n, int type) {
        for (int i = 0; i < sides.length; i++) {
            if (i < 7 && type == VertexParams.POLYGON && sides[i] == n) {
                return i;
            }
            if (i >= 7 && type == VertexParams.STAR && sides[i] == n) {
                return i;
            }
            if (i >= 14 && type == VertexParams.ANYSHAPE && sides[i] == n) {
                return i;
            }
        }
        return -1;
    ...
    public int getTypeIndex(int n, int type) {
        for (int i = 0; i < sides.length; i++) {
            if (i < 7 && type == ExternalSymbolsRenderer.POLYS && sides[i] == n) {
                return i;
            }
            if (i >= 7 && type == ExternalSymbolsRenderer.STARS && sides[i] == n) {
                return i;
            }
            if (i >= 14 && type == ExternalSymbolsRenderer.ANYS && sides[i] == n) {
                return i;
            }
        }
        return -1;
    ...
    public void init() {
        ...
        for (int i = 0; i < numSymbols; i++) {
            ...
            if (i < 7) {
                symbol[i] = new VertexPanel(sides[i], VertexParams.POLYGON);
            } else if (i < 14) {
                symbol[i] = new VertexPanel(sides[i], VertexParams.STAR);
            } else {
                symbol[i] = new VertexPanel(sides[i], VertexParams.ANYSHAPE);
            }
