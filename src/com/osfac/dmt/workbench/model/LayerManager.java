...
import com.osfac.dmt.workbench.ui.style.AbstractPalettePanel;
...
public class LayerManager {
    ...
    //ConcurrentModificationExceptions can occur. [Bob Boseko]
    //Go with List rather than name-to-category map, because category names can
    //now change. [Bob Boseko]
    ...
    public Color generateLayerFillColor() {
        //<<TODO>> Ensure that colour is not being used by another layer [Bob Boseko]
    ...
    public void addLayerable(String categoryName, Layerable layerable) {
        ...
        //Fire metadata changed so that the visual modified markers are updated. [Bob Boseko]
    ...
    private void reproject(Layer layer, CoordinateSystem coordinateSystem) {
        try {
            Assert.isTrue(indexOf(layer) == -1,
                    "If the LayerManager contained this layer, we'd need to be concerned about rolling back on an error");
            ...
            //the CoordinateSystem to the new value [Bob Boseko]
            layer.getFeatureCollectionWrapper().getFeatureSchema()
    ...
    /**
     * @param layerName the name of the layer. A number will be appended if a
     * layer with the same name already exists. Set to null to automatically
     * generate a new name.
     */
    ...
    public String uniqueLayerName(String name) {
        ...
        //<<TODO:REMOVE>> debug [Bob Boseko]
        if ("Relative Vectors".equals(name)) {
    ...
    private void fireLayerEvent(Runnable eventFirer) {
        ...
        //them on the event dispatching thread.[Bob Boseko]
        try {
        ...
        //GUIUtil#invokeOnEventThread. [Bob Boseko]
    }
    ...
    //LayerCategory, layerIndex) can use this simpler method instead. [Bob Boseko]
    ...
    public Iterator iterator() {
        ...
        //Layers first (unlike #getLayers) [Bob Boseko]
    ...
    //they can be replaced by #iterator [Bob Boseko]
    ...
    public List getLayerables(Class layerableClass) {
        ...
        //Create new ArrayLists to avoid ConcurrentModificationExceptions. [Bob Boseko]
    ...
    /**
     * The old method dispose(Layerable layerable) is deprecated. It has been
     * replaced by dispose(WorkbenchFrame frame, Layerable layerable) I add it
     * again for compatibility issues with some older plugins (Pirol's raster
     * PlugIn) I'll remove this method as soon as no more plugin use it
     *
     * @deprecated
     */
    ...
    /**
     * SIGLE [obedel] on 2005 then [mmichaud] on 2007-05-22 To free the memory
     * allocated for a layer Called by RemoveSelectedLayersPlugin
     *
    ...
    public void dispose() {
        ...
        //#die to each to request that the resources be freed. [Bob Boseko]
    ...
    public void setCoordinateSystem(CoordinateSystem coordinateSystem) {
        ...
        //available to this LayerManager (but would be available to a plug-in) [Bob Boseko]        
    }
