...
import com.osfac.dmt.workbench.model.AbstractVectorLayerFinder;
...
public abstract class AbstractDeleteVectorTool extends SpecifyFeaturesTool {
    ...
    public AbstractDeleteVectorTool() {
        super();
        //The cursor is big and the pin icons are big, so make the click buffer big. [Bob Boseko]
    ...
    private Collection centres(Collection vectorFeatures) {
        ...
        for (Iterator i = vectorFeatures.iterator(); i.hasNext();) {
            ...
            //<<TODO>> Envelope shouldn't be null. Use assert instead of if. [Bob Boseko]
            if (envelope.isNull()) {
                continue;
            }
