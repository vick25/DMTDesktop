...
import com.osfac.dmt.geom.LineSegmentEnvelopeIntersector;
...
import com.osfac.dmt.workbench.ui.plugin.scalebar.IncrementChooser;
import com.osfac.dmt.workbench.ui.plugin.scalebar.MetricSystem;
import com.osfac.dmt.workbench.ui.plugin.scalebar.RoundQuantity;
...
public class ZoomBar extends JPanel implements Java2DConverter.PointConverter {
    ...
    public void updateComponents() throws NoninvertibleTransformException {
        ...
        //Must set slider value *before* updating the label on the right. [Bob Boseko]
        //I'm currently hiding the label on the right, to save real estate. [Bob Boseko]
        slider.setValue(
                toSliderValue(
    ...
    private Envelope proposedModelEnvelope() throws NoninvertibleTransformException {
        ...
        if (proposedModelEnvelope.getWidth() == 0 || proposedModelEnvelope.getHeight() == 0) {
            //We're zoomed waaay out! Avoid infinite scale. [Bob Boseko]
    ...
    private void clearModelCaches() {
        ...
        //to #updateComponents [Bob Boseko]
    ...
    private double getMinExtent() throws NoninvertibleTransformException {
        if (modelBlackboard().get(MIN_EXTENT_KEY) == null) {
            ...
            //points (i.e. no segments). [Bob Boseko]
    ...
    //be several zoom bars [Bob Boseko]
    ...
    private void updateSliderLabels() throws NoninvertibleTransformException {
        //Expensive if the data cache has been cleared. [Bob Boseko]
        if (!showingSliderLabels) {
            return;
        }
        if (!(slider.getUI() instanceof BasicSliderUI)) {
        ...
        if (labelTable.isEmpty()) {
            //Get here during initialization. [Bob Boseko]
            return;
        }
        slider.setLabelTable(labelTable);
    }
