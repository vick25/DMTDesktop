...
/**
 * This thread queue executes at maximum N Runnables in parallel were N is a
 * given number of worker threads that should be used. If N threads are running
 * and busy each further incoming Runnable is queued until one of the threads
 * has finished its current job. If a worker thread becomes idle (no more job in
 * the queue) it is hold alive for 5 seconds. If during this period of time no
 * new Runnable is enqueued the worker thread dies.
 *
...
public class ThreadQueue {
    ...
    /**
     * Worker thread. Fetches Runnable from the surrounding ThreadQueue
     * instance.
     */
    ...
    /**
     * If the number of running threads goes down to zero implementations of
     * this interface are able to be informed.
     */
    ...
    /**
     * Informs Listeners of the fact that the number of running threads went to
     * zero.
     */
    ...
    /**
     * The number of currently running worker threads. Alias for
     * runningThreads()
     *
     * @return number of currently running worker threads.
     */
    ...
    /**
     * Adds a Runnables to the queue. It will be run in one of the worker
     * threads.
     *
