...
public class SnapVerticesOp {
    ...
    public boolean execute(
        ...
        for (Iterator i = editableLayers.iterator(); i.hasNext();) {
            ...
            if (insertVerticesIfNecessary) {
                ...
                //Target geometry may have had a vertex inserted. [Bob Boseko]
        ...
        if (target == null) {
            //Can get here if targetFeature is not on the editable layer. [Bob Boseko]
            panel.getContext().warnUser(NO_TARGET_VERTICES_IN_FENCE_WARNING);
            return false;
        }
                ...
                try {
                    ...
                    Assert.isTrue(
                        ...
                        "I assumed that we would get here only if too few points "
                            + "were passed into the Geometry constructor [Bob Boseko]");
                    proposedGeometry =
                        new Point(
                            target,
    ...
    private int insertVerticesIfNecessary(
        ...
        throws NoninvertibleTransformException {
        //Trick: Wrap count in array to avoid "must be declared final" warnings. [Bob Boseko]
        ...
        for (Iterator<Feature> i = transaction.getFeatures().iterator() ; i.hasNext(); ) {
            ...
            //Geometry or GeometryCollection/Polygon element. [Bob Boseko]
            ...
            transaction.setGeometry(feature, geometryEditor.edit(
                ...
                public Geometry edit(Geometry geometry) {
                    if (geometry instanceof Polygon) {
                        //Wait for the individual LinearRings to come in. [Bob Boseko]
                        return geometry;
                    }
                    ...
                    if (!fence.intersects(geometry)) {
                        //A part of the feature that doesn't lie inside the fence. [Bob Boseko]
                        return geometry;
                    }
                    if (!VerticesInFencePlugIn
                    ...
                    //a segment that doesn't intersect the fence. [Bob Boseko]
