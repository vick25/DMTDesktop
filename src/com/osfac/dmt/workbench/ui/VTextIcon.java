 ...
 * component which supports Icons, such as JLabel or JButton <P> You can provide
 * a hint to indicate whether to rotate the string to the left or right, or not
 * at all, and it checks to make sure that the rotation is legal for the given
 * string (for example, Chinese/Japanese/Korean scripts have special rules when
 * drawn vertically and should never be rotated)
 *
 * Rucker, Lee Ann. "Vertical Text in Java." March 2002. Available from
...
public class VTextIcon implements Icon, PropertyChangeListener {
    ...
    /**
     * Creates a
     * <code>VTextIcon</code> for the specified
     * <code>component</code> with the specified
     ...
     * <code>VTextIcon</code> for the specified
     * <code>component</code> with the specified
     * <code>label</code>. It sets the orientation to the provided value if it's
     * legal for the string
     *
     * @see #verifyRotation
     */
     ...
     * employ a horizontal baseline that is rotated by 90deg counterclockwise so
     * that the characters are ordered from top to bottom. Latin text and
     * numbers may be rotated 90deg clockwise so that the characters are also
