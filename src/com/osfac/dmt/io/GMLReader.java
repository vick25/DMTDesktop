 ...
 *
 * <p> DataProperties for the JCSReader load(DataProperties) interface: </p> <p>
 * <table border='1' cellspacing='0' cellpadding='4'> <tr> <th>Parameter</th>
 * <th>Meaning</th> </tr> <tr> <td> File or DefaultValue </td>
 *
 * <td> File name for the input .xml file </td> </tr>
 *
 * <tr> <td> InputTemplateFile </td> <td> Filename for the GMLInputTemplate .xml
 * file </td> </tr>
 *
 * <tr> <td>CompressedFile</td> <td> File name (a .zip or .gz) with a
 * .jml/.xml/.gml inside (specified by File) </td> </tr>
 *
 * <tr> <td> CompressedFileTemplate</td><td>File name (.zip or .gz) with the
 * input template in (specified by InputTemplateFile) </td> </tr> </table> </p>
 *
 * <br> NOTE: If InputTemplateFile is unspecified, GMLReader will try to read
 * one off the top of the input .xml file (the JCS format). This is the same as
 * specifying the File and TemplateFile to be the same. <br> <br>
 *
 ...
 * </pre> <br> <br> Internal Details - This is based on a small finite state
 * machine with these states: <br> <br> STATE MEANING <br> 0 Init <br> 1 Waiting
 * for Collection tag <br> 2 Waiting for Feature tag <br> 3 Getting jcs columns
 * <br> 4 Parsing geometry (goes back to state 3) <br> 1000 Parsing
 * Multi-geometry, recursion level =1 <br> 1001 Parsing Multi-geometry,
 * recursion level =2 <br> ... <br> <br> <br> State Diagram <br> <br> init <br>
 * <PRE>
 *        0  ----->  1
 *                   |
 ...
 *        4<-------->3
 *           Geometry start/end
 * </PRE> <br> For multi-geometries <br> On start Multi-geometry, increment
 * state by 1 (or jump to 1000 if at state 4) <br> make sure
 * recursivegeometry[state-1000] is null <br> <put any object into the
 * recursivegeometry[state-1000] collection>
 *
 * <br> <br> on end multi-geometry, <br> build geometry in
 * recursivegeometry[state-1000], add it to recursivegeometry[state-1000-1] <br>
 * state= state-1 <br> <br> For single geometries - they will be stuck into
 * recursivegeometry[0], which is the same <br> as geometry <br> <br> For multi
 * geometries - they will also be stuck into recursivegeometry[0], which is the
 * same <br> as geometry. But, for the first nested geometry, it will be stuck
 * into recursivegeometry[1], <br> which will then be geometry <br>
 * <pre>
 ...
 * </pre>
 *
 * Most of the work is done in the endTag method! <br> <br> New additions: Jan
 * 2005 by Dave Blasby allow srid to be parsed from the GML file For example:
 * <gml:LineString srsName="EPSG:42102"> .... </gml:LineString> The SRID of the
 * created geometry will be 42102. It accepts srsNames of the form
 * "<letters>:<number>". ie. "EPSG:111" or "DAVE:222" or "BCGOV:333" etc... The
 * Geometry's SRID will be the number. If you have a GEOMETRYCOLLECTION with
 * more than one SRID specified the SRID of the result will be indeterminate
 * (this isnt correct GML).
 *
...
public class GMLReader extends DefaultHandler implements JUMPReader {
    ...
    /**
     * STATE MEANING <br> 0 Init <br> 1 Waiting for Collection tag <br> 2
     * Waiting for Feature tag <br> 3 Getting jcs columns <br> 4 Parsing
     * geometry (goes back to state 3) <br> 1000 Parsing Multi-geometry,
     * recursion level =1 <br> 1001 Parsing Multi-geometry, recursion level =2
     * <br>
     */
     ...
     * processMultiItems as lists) <a><b>...1...</b></a> --> b get the string
     * "b" (as before) <a><b>...1...</b><b>...2...</b><b>...3...</b></a> --> 'b'
     * is a list of String ['...1...','...2...','...3...']
     */
