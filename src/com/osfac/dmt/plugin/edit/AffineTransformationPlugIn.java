...
public class AffineTransformationPlugIn extends AbstractThreadedUiPlugIn {
    ...
    public void run(TaskMonitor monitor, PlugInContext context) throws Exception {
        ...
        AffineTransformation toOriginTrans = AffineTransformation.translationInstance(-originX, -originY);
        trans.compose(toOriginTrans);

        if (scaleX != 1.0 || scaleY != 1.0) {
            AffineTransformation scaleTrans = AffineTransformation.scaleInstance(scaleX, scaleY);
        ...
        if (rotationAngle != 0.0) {
            AffineTransformation rotTrans = AffineTransformation.rotationInstance(Math.toRadians(rotationAngle));
            trans.rotate(Math.toRadians(rotationAngle));
        }

        AffineTransformation fromOriginTrans = AffineTransformation.translationInstance(originX, originY);
        trans.compose(fromOriginTrans);

        if (transX != 0.0 || transY != 0.0) {
            AffineTransformation translateTrans = AffineTransformation.translationInstance(transX, transY);
